...
let promise = new Promise((resolve,reject)=>{
  ...
  for (let i = 0; i<clumps.length;i++) {
    console.log(clumps[i]['name']);


    // clump is an array of LineBuilder objects, taking the names of the pull, [1,2,3] the data points, stop ids and stop names
    for (let j = 0 ; j<clumps[i]['stopNames'].length;j++) {
      if(clumps[i]['name'].includes('1') && lineData['line1'].includes(clumps[i]['stopNames'][j])) {
        lines[0].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('2') && lineData['line2'].includes(clumps[i]['stopNames'][j])) {
        lines[1].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('3') && lineData['line3'].includes(clumps[i]['stopNames'][j])) {
        lines[2].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('4') && lineData['line4'].includes(clumps[i]['stopNames'][j])) {
        lines[3].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('5') && lineData['line5'].includes(clumps[i]['stopNames'][j])) {
        lines[4].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('6') && lineData['line6'].includes(clumps[i]['stopNames'][j])) {
        lines[5].push(clumps[i]['data'][j]);
      }
      if(clumps[i]['name'].includes('7') && lineData['line7'].includes(clumps[i]['stopNames'][j])) {
